...
'''
													...
													#//Login dan Cookies Defs\\#
												# /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\ #
# //X IG APP ID
def random_app_id():
	return ''.join(choice(string.digits) for _ in range(15))
# //Insta AJAX
def random_AJAX():
	return ''.join(choice(string.digits + string.ascii_lowercase) for _ in range(12))
#Search Token (Mencari token) 
token = get('https://www.instagram.com/accounts/login/?source=auth_switcher').content.split('csrf_token":"')[1].split('"')[0]
...
def log(u, p):
	...
	r = post('https://www.instagram.com/accounts/login/ajax/', data=data, headers=headers)
	src = r.content
	# //print src
...
def cookiesmaker():
	...
	#Lokasi
	f = get('https://ipapi.co/');f = f.content; ip = f.split('class="key">IP Address</td><td class="ipval" data-clipboard-text="')[1].split('"')[0]
	global cookie
...
def like(item):
	global headers
	try:
		q = get(item); q = q.content; item_id = q.split('instagram://media?id=')[1].split('"')[0]
		x = post('https://www.instagram.com/web/likes/'+item_id+'/like/', headers=headers)
		xsrc = x.content
		# //print xsrc
...
def comment(item, msg):
	try:
		...
		x = post('https://www.instagram.com/web/comments/'+item_id+'/add/', data=data, headers=headers)
		xsrc = x.content
		# //print xsrc
		if '"status": "ok"' in xsrc:
...
def follow(item):
	global headers
	try:
		q = get(item); q = q.content; item_id = q.split('logging_page_id":"profilePage_')[1].split('"')[0]
		
		x = post('https://www.instagram.com/web/friendships/'+item_id+'/follow/', headers=headers)
		xsrc = x.content
		# //print xsrc
		if '"status": "ok"' in xsrc:
...
def report(item):
	try:
		...
		x = post('https://www.instagram.com/web/users/'+item_id+'/report/', data=data, headers=headers)
		xsrc = x.content
		# //print xsrc
		if '"status": "ok"' in xsrc:
